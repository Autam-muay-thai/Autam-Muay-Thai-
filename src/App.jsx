...
function CoachIAScreen({ isPremium, dailyAnalyses, setDailyAnalyses, goPaywall }) {
  return (
    <div style={{ padding: "20px 16px 80px", height: "100%", overflowY: "auto", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      <div style={{ fontSize: 11, color: "#ef4444", letterSpacing: 3, fontFamily: "'Bebas Neue', cursive", marginBottom: 16 }}>
        COACH IA
      </div>
      <div style={{ fontSize: 64, marginBottom: 16 }}>🤖</div>
      <div style={{ fontSize: 28, fontFamily: "'Bebas Neue', cursive", color: "#fff", letterSpacing: 2, marginBottom: 12, textAlign: "center" }}>
        BIENTÔT DISPONIBLE
      </div>
      <div style={{ fontSize: 14, color: "#888", lineHeight: 1.6, textAlign: "center", maxWidth: 280, marginBottom: 24 }}>
        Le Coach IA arrive très bientôt ! Il pourra analyser tes vidéos d'entraînement et te donner des corrections personnalisées en direct.
      </div>
      <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 14, padding: 14, maxWidth: 300 }}>
        <div style={{ fontSize: 12, color: "#ef4444", fontFamily: "'Bebas Neue', cursive", letterSpacing: 1, marginBottom: 6 }}>
          🔥 EN ATTENDANT
        </div>
        <div style={{ fontSize: 12, color: "#aaa", lineHeight: 1.6 }}>
          Profite des techniques détaillées, programmes complets, du timer pro, des défis du jour et du suivi de progression !
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: "#0a0a0a" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 22, fontFamily: "'Bebas Neue', cursive", color: "#ef4444", letterSpacing: 2, marginBottom: 8 }}>BIENTÔT DISPONIBLE</div>
        <div style={{ fontSize: 13, color: "#888", marginBottom: 24, lineHeight: 1.6, maxWidth: 280 }}>
          Le Coach Autam arrive très bientôt ! Tu pourras lui poser toutes tes questions sur le Muay Thai, la nutrition et ta progression.
        </div>
        <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 14, padding: 14, maxWidth: 300 }}>
          <div style={{ fontSize: 12, color: "#ef4444", fontFamily: "'Bebas Neue', cursive", letterSpacing: 1, marginBottom: 6 }}>🔥 EN ATTENDANT</div>
          <div style={{ fontSize: 12, color: "#aaa", lineHeight: 1.6 }}>Découvre les techniques, programmes, défis et suivi de progression !</div>
        </div>
      </div>
    </div>
  );
...
function DefiDuJourScreen({ setScreen, isPremium, goPaywall, onBadge, onShare, isOnline }) {
  ...
  const generateDefi = async () => {
    ...
    setLoading(true);
    // Use offline defis (no API needed)
    ...
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          system: `Tu es un coach Muay Thai expert qui crée des défis d'entraînement quotidiens personnalisés. Réponds UNIQUEMENT en JSON valide avec cette structure exacte:
{"titre":"<nom accrocheur du défi>","emoji":"<1 emoji>","objectif":"<une phrase motivante>","niveau":"<niveau>","combo":["<frappe1>","<frappe2>","<frappe3>"],"rounds":<nombre>,"duree_round":<secondes>,"repos":<secondes>,"exercices":[{"nom":"<exercice>","reps":"<reps ou durée>","desc":"<conseil technique court>"}],"conseil_coach":"<conseil motivant et technique>","calories_estimees":<nombre>}`,
          messages: [{
            role: "user",
            content: `Crée un défi du jour en Muay Thai pour niveau ${level}.
Progression de l'élève: ${statsInfo}.
Date: ${new Date().toLocaleDateString("fr-FR")}.
Adapte la difficulté au niveau ${level}.
Le combo doit avoir 3-5 frappes.
Les exercices doivent être 3-4 exercices complémentaires au Muay Thai.
